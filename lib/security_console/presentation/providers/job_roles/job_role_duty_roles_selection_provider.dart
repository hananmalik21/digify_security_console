...
class JobRoleDutyRolesSelectionState {
  const JobRoleDutyRolesSelectionState({
    this.roles = const [],
  ...
  JobRoleDutyRolesSelectionState copyWith({
    ...
    return JobRoleDutyRolesSelectionState(
...
class JobRoleDutyRolesSelectionNotifier extends StateNotifier<JobRoleDutyRolesSelectionState> {
  JobRoleDutyRolesSelectionNotifier(this._useCase, this._ref) : super(const JobRoleDutyRolesSelectionState());
  ...
  Future<void> _fetchPage(int page, {bool resetSearch = false}) async {
    ...
    try {
      ...
      final result = await _useCase(enterpriseId: enterpriseId, search: search, page: page, pageSize: state.pageSize);
      final items = result.roles.map(DutyRoleItem.fromDutyRole).toList();
      final newCache = {...state.codeToIdCache, for (final r in items) r.code: r.id};
...
final jobRoleDutyRolesSelectionProvider =
    StateNotifierProvider.autoDispose<JobRoleDutyRolesSelectionNotifier, JobRoleDutyRolesSelectionState>((ref) {
      ...
      return JobRoleDutyRolesSelectionNotifier(useCase, ref);
    });
