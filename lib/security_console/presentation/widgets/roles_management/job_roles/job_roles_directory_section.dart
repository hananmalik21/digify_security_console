...
import 'package:digify_security_console/security_console/presentation/widgets/roles_management/job_roles/job_roles_list_skeleton.dart';
import 'package:digify_security_console/security_console/presentation/widgets/roles_management/job_roles/job_roles_role_card.dart';
...
class JobRolesDirectorySection extends StatelessWidget {
  const JobRolesDirectorySection({
    super.key,
    required this.state,
...
class _Body extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (state.isLoading) {
      return const JobRolesListSkeleton();
    }
    ...
    if (roles.isEmpty) {
      return const RolesManagementEmptyBody(message: 'No job roles found for the current search and filter.');
    }

    return Column(
      children: [
        for (int i = 0; i < roles.length; i++) ...[JobRoleCard(role: roles[i]), if (i != roles.length - 1) Gap(14.h)],
      ],
    );
  }
}
    ...
    if (context.isMobile) {
      return Column(
        ...
        children: [
          Row(
            children: [
              Text('Job Roles Directory', style: context.textTheme.titleMedium?.copyWith(color: AppColors.textPrimary)),
              Gap(8.w),
              ...
              if (canCreateRole) ...[
                ...
                Expanded(
                  child: AppButton.primary(
                    ...
                    onPressed: () => CreateJobRoleDialog.show(context),
                    icon: Icons.add,
                    width: double.infinity,
                  ),
                ),
